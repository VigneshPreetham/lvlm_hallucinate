...
class RAM_plus(nn.Module):
    def __init__(self,
                 med_config=f'../configs/med_config.json',
                 image_size=384,
                 ...
                 threshold=0.68,
                 delete_tag_index=[],
                 tag_list=f'ram/configs/ram_tag_list.txt',
                 tag_list_chinese=f'ram/configs/ram_tag_list_chinese.txt',
                 stage='eval'):
        ...
        if vit == 'swin_b':
            if image_size == 224:
                vision_config_path = f'../configs/swin/config_swinB_224.json'
            elif image_size == 384:
            ...
            if image_size == 224:
                vision_config_path = f'ram/configs/swin/config_swinL_224.json'
            elif image_size == 384:
                vision_config_path = f'ram/configs/swin/config_swinL_384.json'
            vision_config = read_json(vision_config_path)
            assert image_size == vision_config['image_res']
            vision_width = vision_config['vision_width']
        ...
        q2l_config = BertConfig.from_json_file(f'ram/configs/q2l_config.json')
        ...
        if stage == 'train_from_scratch':
            self.label_embed = nn.Parameter(torch.load(f'../data/frozen_tag_embedding/ram_plus_tag_embedding_class_4585_des_51.pth',map_location='cpu').float())
        else:
        ...
        ram_class_threshold_path = f'ram/configs/ram_tag_list_threshold.txt'
